...
#if ENABLE_DATALESS_PLAYER_GUI
                EditorUserBuildSettings.datalessPlayer = EditorGUILayout.Toggle(styles.datalessBuild, EditorUserBuildSettings.datalessPlayer);
#endif
...
#if INCLUDE_IL2CPP_PUBLIC
            if (BuildPipeline.DoesBuildTargetSupportIl2cpp(EditorUserBuildSettings.selectedStandaloneTarget)
                && platform.targetGroup != BuildTargetGroup.iOS)
            {
                bool il2cpp = IsIl2cppBuild();
                il2cpp = GUILayout.Toggle(il2cpp, styles.useNativeCompilation);
                PlayerSettings.SetScriptingBackend(EditorUserBuildSettings.selectedBuildTargetGroup, (il2cpp ? ScriptingImplementation.IL2CPP : GetDefaultNonIl2cppBackend()));
            }
#endif
